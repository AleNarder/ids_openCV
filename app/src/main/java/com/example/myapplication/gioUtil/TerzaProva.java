...
import com.example.myapplication.MyNearby;
...
public class TerzaProva {
    ...
    public TerzaProva(Context context , TachoMaster tachoMaster , FloorMaster floorMaster , SensorMaster sensorMaster,
                      List<Floor.OnFloorPosition> minePosition , FirstTryActivity.MyCameraListener cameraListener, MyNearby nearby){
    ...
    public void findMine() throws InterruptedException, ExecutionException, IOException, N_ReachableException {
        ...
        while(!sensorMaster.objectInProximity()) {
            if (noDestination) {
                destination = minePosition.remove(0);
                noDestination = false;
            }
